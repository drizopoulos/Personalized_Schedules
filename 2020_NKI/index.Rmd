...
$$\mathcal N_j(\kappa^*) = \sum_{n = 1}^N \mbox{I}\{\pi_j(s_n \mid t_n, v) \geq \kappa^*\} \times \{1 - \pi_j(t_{n-1} \mid t_{n-2}, v)\}$$
...
- For a specific threshold $\kappa^*$ we can calculate
  + the expected delay
  $$\begin{array}{lcl}
  \mathcal D_j(\kappa^*) & = & \sum\limits_{n = 1}^N \bigl \{ t_n - E(T_j^* \mid t_{n-1} \leq T_j^* \leq t_n) \bigr \} \times\\&&\\ && \quad \quad \quad \Pr(t_{n-1} \leq T_j^* \leq t_n)
  \end{array}$$
...
- Schedules become more personalized the better biomarkers distinguish patients
  + consider more biomarkers, e.g., for postate cance MRI
