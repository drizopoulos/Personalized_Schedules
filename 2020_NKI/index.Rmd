...
<div style="text-align:center;width:600px;border:3px solid black">
<br/>
<strong><font size="6" color = "red">How to we select ${\color{red} \kappa_n}$?</font></strong>
<br/>
</div>
...
$$\mathcal N_j(\kappa^*) = \sum_{n = 1}^N \mbox{I}\{\pi_j(s_n \mid t_n, v) \geq \kappa\} \times \{1 - \pi_j(s_n \mid t_n, v)\}$$
...
$$\mathcal D_j(\kappa^*) = \sum_{n = 1}^N \bigl \{ s_n - E(T_j^* \mid s_{n-1} \leq T_j^* \leq s_n) \bigr \} \times \Pr(s_{n-1} \leq T_j^* \leq s_n)$$
...
- For different $\kappa$'s we can calculate the two-dimensional space
...
$$\kappa_n^{opt} = \mbox{argmin}_{\kappa} \sqrt{ \bigl \{ \mathcal N_j(\kappa) - 1 \bigr \}^2 + \mathcal D_j(\kappa)^2}$$
...
risk_plot = baseggplot_no_xticks +
  ...
  scale_shape_manual(name="",
                     labels= c(expression(atop('Observed', 'DRE')), 
                               expression(atop('Observed', 'log'[2]*'(PSA + 1)'))),
                     values = c(17, 16)) +
  scale_linetype_manual(name="",
                        labels= c(expression(atop('Fitted', 'Pr (Palpable DRE)')), 
                                  expression(atop('Fitted', 'log'[2]*'(PSA + 1)'))),
                        values = c("dotted", "dashed")) +
  ylab(expression('Pr(DRE)       log2(PSA+1)')) +
  #ylab('DRE              PSA') +
  xlab("Follow-up time (years)") + 
  xlim(min_x,MAX_FOLLOW_UP) +
  theme(axis.text.y.right = element_text(size=8, color=DANGER_COLOR),
        axis.title.y.right = element_text(size=10, color=DANGER_COLOR)) +
...
schedule_plot = baseggplot +
  ...
  geom_label(data=schedule_df, label.padding = unit(0.16, "lines"),
...
final_plot = ggpubr::ggarrange(upper_plot, lower_plot, 
                               nrow=2, ncol=1, align = "v", labels = NULL)
